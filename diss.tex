...
\usepackage{tikz}
\usetikzlibrary{backgrounds,shapes,arrows,fit}
...
\todo{Make Ypnos prims a subset of Haskell Code.}
\begin{figure}[!h]
\centering
% Define block styles
...
\begin{tikzpicture}[row sep=0.5em]
  ...
  \begin{pgfonlayer}{background}
    \node [surround] (background1) [fit = (title1) (libs)] {};
    \node [surround] (background2) [fit = (title2) (libs2)] {};
...
minimal\footnote{Basic guarantees are given but most of the work is left to the
  programmer.}\todo{Expand?}, which means that operations on the different cores
...
Figure~\ref{fig:copyonoff})\todo{Ensure this figure is in the right place.}.
...
The map function is depicted in Listing~\ref{lst:map}.\todo{compare run and map? Comonads?}
...
\end{table}
\todo{top bottom lines, make wider}
...
arising from Category Theory\cite{uustalu2008}\todo{Is this fair use?}. The run
...
Section~\ref{sec:typesysapp})\todo{Have we?}.  To solve this we previously broke the
...
require minimal code changes\todo{Mention this earlier}.  With the previous
...
as the \texttt{run} and \texttt{reduce} primitives.\todo{Stress user doesn't see
  crazy type stuff.}
...
\todo{Refer back to iteration phases.}Recall from Section~\ref{sec:introduction-ypnos} that Ypnos is a collection of
...
destructors.\todo{Ensure we get a double page here}
...
discounted this.\todo{Why have I discounted? ref?}
...
\begin{itemize}
\itemsep1pt\parskip0pt\parsep0pt
\item \textbf{The Average Stencil} (5-point version) \todo{Reference avg
    stencil} that we have seen in the previous sections. This function is
...
\item \textbf{The Game of Life Stencil}\todo{Put code in appendices} makes use
...
\item \textbf{The Total Reducer}\todo{Reference} when normalised gives the mean
...
fits together.\todo{Compare with requirements to wrap up?}
