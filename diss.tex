...
many dimensionalities (1D, 2D, 3D, etc.). \todo{Explain parallelism due to
  stencils?}
...
Figure~\ref{fig:copyonoff})\todo{make show in right place}.  .
...
\end{figure}\todo{change coff->padstart roff->padend}
...
\end{figure}\todo{colour under copy-on/off}
...
following three approaches\todo{Enough just to have references without
  examples?}: choosing the different implementation based on importing (also
...
(Sections~\ref{sec:assoctypefam} and \ref{sec:final}) For the sake of comparison
I will also include the approach of the programmer re-coding their
